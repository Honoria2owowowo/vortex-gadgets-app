...
(function () {
  ...
  function vProduct(handle) {
    ...
    return '<div class="pdp" data-handle="' + esc(p.handle) + '">' +
      ...
      urgenciaHtml() +
      /* [2026-09-22] Quien vende, en la propia ficha. El dato ya estaba en el pie y
  ...
  var HERO_SLIDES = [
    ...
    { mod: "hero-slide--sinfoto", style: "background:#ffe600;",
  ...
  /* Precios y descuentos en vivo para el carrusel.
     Tokens: [[P:handle|respaldo]] precio · [[O:handle|respaldo]] etiqueta de oferta.
     Si el producto no esta cargado o no tiene precio anterior, queda el respaldo:
     asi nunca se ve vacio ni un precio a medias. */
  function productoPorHandle(h) {
    for (var i = 0; i < state.products.length; i++) {
      if (state.products[i].handle === h) return state.products[i];
    }
    return null;
  }
  ...
  function htmlCorte() {
    ...
    /* [2026-09-23] Devuelve un <span>, no un <div>: este contador ahora va DENTRO de la
  ...
  function pintarCortes() {
    ...
    for (var i = 0; i < els.length; i++) {
      ...
      /* [2026-09-23] Texto mas corto (antes: "y tu pedido sale hoy") porque ahora va en
         linea con la letra fina. Y en la ULTIMA HORA cambia solo a "Quedan 42 min · sale
         hoy": esa es la urgencia de minutos que pedia el dueño, pero REAL — nace de su
         hora de corte, no de un reloj inventado que se reinicia. */
      var min = Math.max(1, Math.ceil(ms / 60000));
      els[i].innerHTML = (hoy
        ? (min <= 60
          ? 'Quedan ' + min + ' min · <b>sale hoy</b>'
          : 'Pide antes de las ' + reloj12(hh, mm) + ' y <b>sale hoy</b>')
        : 'Ya cerró el despacho · <b>sale en el siguiente</b>') +
  ...
  function heroPreciosVivos(html) {
    ...
    return html.replace(/\[\[([PO]):([^|\]]+)\|([^\]]*)\]\]/g, function (m, tipo, h, respaldo) {
      ...
      if (tipo === 'P') return money(p.price);
      if (!(p.compare > p.price)) return respaldo;
      var ahorro = p.compare - p.price;
      return '-' + Math.round(ahorro / p.compare * 100) + ' % · <s>' + money(p.compare) +
             '</s> · Ahorras ' + money(ahorro);
    });
  }
  function vHeroSlider() {
    ...
    /* [2026-09-22] Los precios del carrusel se leen EN VIVO del catalogo.
       Antes estaban escritos a mano y, si se cambiaba un precio en Shopify, la
       diapositiva seguia mostrando el viejo. Los tokens [[P:handle|respaldo]] y
       [[O:handle|respaldo]] se sustituyen con el precio y el descuento reales; si
       el producto todavia no esta cargado, se queda el respaldo. */
    inner = heroPreciosVivos(inner);
